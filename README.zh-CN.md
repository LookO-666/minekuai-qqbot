...
- **管理员更换整合包**：在 QQ 中搜索整合包、选择版本，再通过一次性清空确认；持久维护保护需管理员核对官网后显式解除
...
| 🔒 `确认清空安装 <6位码>` | 5 分钟内确认清空并安装；必须由原 QQ 用户在同一个 Bot、同一群或同一私聊会话中回复 |
| 🔒 `取消更换整合包` | 取消自己尚未提交的选择／确认；不会中止已提交的安装，也不会解除维护保护 |
| 🔒 `整合包状态 [服务器]` | 查看整合包／维护状态；“已提交”不等于“安装完成” |
| 🔒 `结束整合包维护 <服务器> 我已核对` | 在官网确认安装已结束或失败后，显式解除维护保护；不会取消或完成远端安装 |
...
**此操作会覆盖服务器全部文件，包括世界存档。确认前请自行备份需要保留的内容，机器人不会自动备份。**

1. 自行备份并停止实例。机器人不会强杀运行中的实例，也不会为了安装而开启计时卡计费。
2. 发送 `更换整合包 <服务器> <关键词>`，按交互提示搜索、翻页、选择整合包和版本。版本 `0` 仅表示采用主目录条目当前展示的版本。
3. 核对最终警告中的服务器、整合包、版本、Minecraft 版本和 Java 版本，5 分钟内在**原会话**发送 `确认清空安装 <确认码>`。群里产生的安装确认不能转到私聊或另一个群回复。确认码只能使用一次；服务器身份或目录选择发生变化时，需要重新确认。
4. **已提交仅表示安装请求已提交，不代表安装完成。** 超时或结果未知时不会自动重试写操作。受影响服务器／计时卡会进入持久维护保护，阻止开关服、计时卡操作、重启、控制台指令、QQ → Minecraft 聊天写入及相关配置修改，后台操作也受保护约束。
5. 到[麦块官网](https://minekuai.com)检查实际进度。确认安装已经结束或失败后，再发送 `结束整合包维护 <服务器> 我已核对`。此指令只解除机器人的保护，不代表服务器运行正常，也不会恢复备份、开服或停止远端安装器。
...
7. **开启群全员管理员等于完全信任全体群员**：除非每位成员都可以管理凭据、执行控制台命令、关停或删除服务器，以及确认会清空全部文件和世界的整合包更换，否则保持 `ADMIN_ALL_GROUP_MEMBERS=false`
8. **定期备份 SQLite**：
   ```bash
