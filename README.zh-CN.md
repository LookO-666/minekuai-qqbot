...
4. 机器人先建立维护保护并开启计时卡。**平台可能随开卡自动启动实例**；如果是本次开卡触发的启动，机器人最多发送一次正常停服指令并等待离线，不会强杀。只有只读确认实例已解冻且离线后，才发送一次覆盖安装请求。开卡、正常停服或就绪检查失败时，不提交安装，维护保护仍然保留。结果未知的写操作不会自动重试。保护会阻止其他开关服、计时卡操作、重启、控制台指令、QQ → Minecraft 聊天写入及相关配置修改，后台操作也受约束。
