...
- **管理员更换整合包**：在 QQ 中搜索整合包、选择版本，明确确认授权开卡计费与清空安装；持久维护保护需管理员核对官网后显式解除
...
| 🔒 `整合包状态 [服务器]` | 查看整合包／维护状态；“已提交”不等于“安装完成”，计费可能仍开启，需到官网核对 |
...
5. **已提交仅表示安装请求已提交，不代表安装完成。** 到[麦块官网](https://minekuai.com)检查实际进度与计费。确认安装已经结束或失败后，按需手动关闭计时卡，再发送 `结束整合包维护 <服务器> 我已核对`。解除保护不会关闭计时卡，不代表服务器运行正常，也不会恢复备份、启动游戏或停止远端安装器。

**此流程不会自动关闭计时卡**，以免中断结果未知的安装；正常停止游戏进程并不等于关闭计时卡。即使报错或超时，计时卡也可能继续开启并消耗时长；请先到官网核对，再手动关卡或解除保护。

维护保护保存在 SQLite 中，机器人重启、删除后重建服务器配置都不会使其消失。保护没有自动过期时间，也不会因为安装接口返回成功就自动解除。`取消更换整合包` 只能取消尚未提交的交互。
