...
- **管理员更换整合包**：在 QQ 中搜索整合包、选择版本，明确授权开卡计费与清空安装，再只读观察本次安装日志／状态；维护保护需管理员核对官网后显式解除
...
| 🔒 `整合包状态 [服务器]` | 只读核对本次维护期间的安装结果与计费：已完成、失败、安装中或未知；记录的选择不等于自动识别当前已安装整合包 |
| 🔒 `结束整合包维护 <服务器> 我已核对` | 在官网确认安装结束并核对计费后解除保护；不会关闭计时卡、取消安装或启动游戏 |
...
5. 提交后，机器人会只读观察安装日志与状态，最长约 10 分钟，因此确认指令可能需要等待几分钟。**已提交不等于已完成**；只有属于本次维护的安装证据才可确认完成，旧的成功日志不足以证明本次成功。API 响应解析异常或响应丢失也不等于安装失败：请求可能已被受理，后续只读观察仍可确认完成。观察超时或机器人重启后，用 `整合包状态 <服务器>` 继续核对，不要重新提交安装。
6. 到[麦块官网](https://minekuai.com)核对安装结果、实际文件与计费。确认安装已经结束或失败后，按需手动关闭计时卡，再发送 `结束整合包维护 <服务器> 我已核对`。解除保护不会关闭计时卡，不代表服务器运行正常，也不会恢复备份、启动游戏或停止远端安装器。记录中的整合包／版本只是本次选择，并非独立识别当前实际安装内容。

**此流程不会自动关闭计时卡**，以免中断结果未知的安装；正常停止游戏进程并不等于关闭计时卡。即使报错或超时，计时卡也可能继续开启并消耗时长；请先到官网核对，再手动关卡或解除保护。

维护保护和安装观察结果保存在 SQLite 中，机器人重启仍保留；删除后重建服务器配置也不会移除保护。保护没有自动过期时间，不会因为接口返回成功或观察到安装完成／失败就自动解除。`取消更换整合包` 只能取消尚未提交的交互。
