...
- **Administrator-only modpack replacement:** search and select a pack/release in QQ, explicitly authorize billing and a clean installation, then observe the current attempt through read-only installation logs/status; maintenance protection remains until an administrator releases it
...
| 🔒 `整合包状态 [server]` | Read the current maintenance attempt's installation evidence and billing state: completed, failed, installing, or unknown. The recorded selection is not automatic detection of the installed pack. |
...
4. The bot establishes maintenance protection and activates the time card. **The platform may automatically start the instance when billing is activated.** If this activation causes the instance to start, the bot sends at most one graceful stop command and waits for it to go offline; it never force-kills it. Startup is not required: an authenticated live WebSocket that remains offline, together with HTTP instance/resource, billing, and status checks, can confirm readiness. The instance must be unsuspended and offline before the single installation request. If activation, graceful shutdown, or readiness checks fail, no installation is submitted and protection remains. Unknown writes are not retried automatically. Protection blocks other power/time-card operations, restarts, console commands, QQ → Minecraft chat writes, and related configuration changes, including background operations.
5. The bot observes installation through read-only logs and state checks for up to ten minutes, so the confirmation command may take several minutes. **Submitted is not completed.** It can report completion when evidence belongs to this maintenance attempt; an old successful log line is not sufficient. An API parsing error or lost response also does not establish failure: the request may have been accepted, and read-only observation can still confirm completion. After an observation timeout or bot restart, use `整合包状态 <server>` to continue checking—do not submit the installation again.
6. Check the result, files, and billing on the [Minekuai website](https://minekuai.com). Once installation has ended or failed, manually stop the time card when appropriate and send `结束整合包维护 <server> 我已核对`. Releasing protection does not stop billing, prove the server is healthy, recover a backup, start the game, or stop a remote installer. The recorded pack/release is what you selected, not an independent identification of the currently installed files.
...
Protection and installation observations are stored in SQLite and survive bot restarts; deletion/recreation of a server configuration does not remove protection. It never expires automatically and is not cleared by a successful API response or an observed completed/failed installation. `取消更换整合包` only cancels an unsubmitted interaction.
