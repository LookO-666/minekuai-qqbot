...
- **Administrator-only modpack replacement:** search and select a pack/release in QQ, explicitly authorize billing and a clean installation, then check logs/status in QQ; confirmed safe completion automatically releases maintenance protection and preserves the result
...
5. The bot observes installation through read-only logs and state checks for up to ten minutes, so the confirmation command may take several minutes. **Submitted is not completed.** Completion requires evidence belonging to this maintenance attempt; an old successful log line is not sufficient. An API parsing error or lost response does not establish failure: the request may have been accepted, and read-only observation can still confirm completion. After a timeout or bot restart, use `整合包状态 <server>` and `整合包日志 <server>` in QQ to continue checking—do not submit the installation again.
...
Protection, observations, and completed maintenance history are stored in SQLite and survive bot restarts; deletion/recreation of a server configuration does not remove active protection. Protection has no time-based expiry: release requires a confirmed completed/failed task with no active installer, or confirmation that no installation was submitted and no task is running. A successful API receipt alone cannot release it. `取消更换整合包` only cancels an unsubmitted interaction.
