...
- **长期面板 API Key**：实例电源、状态、控制台、文件和 WebSocket 优先使用 Pterodactyl Client API Key，cookies 仅作兼容回退
- **聊天交互式配置**：群里指令就能加 / 删 / 改服务器，不用编辑 `.env` + 重启
- **持久化**：所有配置存 SQLite；token、账号密码和会话凭据使用 Fernet 加密；所有操作有日志
- **分级权限 + 冷却 + 二次确认**：普通群友可开关服和查询，管理员负责配置与运维
...
> 默认无前缀（群里直接发『开服』触发）。可在 `.env` 里改 `COMMAND_START`。新增、删除、修改服务器/账号以及日志、重启、控制台等运维指令仅 `ADMIN_USERS` 中的 QQ 可用。
